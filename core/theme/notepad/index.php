<?php if($_SESSION['role'] == "0"):?>
	<div id="right_content">
		<form action="/" method="post" enctype="multipart/form-data">
			...
			<input type="text" placeholder="Заголовок записи" name="post[title]" required>(Заголовок записи)<br>
			<textarea name="post[text]" rows="19" cols="44" placeholder="Текст записи"></textarea>(Текст записи)<br>
			<input accept=".jpg, .jpeg, .png, .gif, .bmp" name="image" type="file" />(Изображение записи)<br>
			<input type="date" value="<?php echo date("Y-m-d");?>" name="post[date_write]" required>(Дата написания)<br>
